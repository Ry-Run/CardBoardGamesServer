...
// Packet 包类型
const (
	None         PackageType = 0x00 // 无
	Handshake    PackageType = 0x01 // 握手 Handshake represents a handshake: request(client) <====> handshake response(server)
	HandshakeAck PackageType = 0x02 // 握手确认，客户端响应收到 Handshake，服务端一般不用处理 HandshakeAck represents a handshake ack from client to server
	Heartbeat    PackageType = 0x03 // 心跳 Heartbeat represents a heartbeat
	Data         PackageType = 0x04 // 数据包 settings represents a common data packet
	Kick         PackageType = 0x05 // 断开连接，服务端会主动发起，所以服务端一般不用处理 Kick represents a kick off packet
)

// body 里面描述数据的消息类型
const (
	Request  MessageType = 0x00 // ----000- 请求
	Notify   MessageType = 0x01 // ----001- 通知
	Response MessageType = 0x02 // ----010- 响应，messageId 必须与 request 过来的 messageId 一致
	Push     MessageType = 0x03 // ----011- 推送
)
...
func Decode(payload []byte) (*Packet, error) {
	...
	if p.Type == Data {
		// 数据包，body 是官方所说的 message 类型，专门进行解码
